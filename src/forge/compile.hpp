...
struct CompileReport {
    // The node in the returned field that answers what `root` answered in the original. It is
    // also always the LAST node of the returned field, so a caller that ignores the report can
    // still find it — but read it from here, because "the last one" is a promise about an
    // implementation and this is a promise about an answer.
    u32 root = 0;
