...
export class Post {
    ...
    // Put the scene target back after a pass that bound one of its own.
    //
    // THE MERGE FOUND THIS AND IT IS THE WHOLE FEATURE. Two passes that landed on other branches --
    // the sun's shadow map and the offscreen scene capture the reflections march -- each render to
    // their own framebuffer and each restore the DEFAULT one when they are done, because when they
    // were written the default one was where the frame went. It is not any more. Between them they
    // unbound this target after the sky was drawn into it, so every surface went to the canvas and
    // then the composite painted an almost-empty target over the top: a page with a sky on it, no
    // console error, and nothing to say which of eleven features did it.
    rebind(width, height) {
