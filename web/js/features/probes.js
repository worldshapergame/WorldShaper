...
export const PROBE_ATLAS_UNIT = 11;  // UNIT.probeAtlas in web/js/gl.js, which is the register
export const PROBE_INDEX_UNIT = 12;  // UNIT.probeIndex

export function readProbes(bytes) {
    // `bytes` is what `clip.chunk('RPRB')` hands back: a zero-copy view of the chunk, or null.
    // The chunk directory is the one settled at the merge and every reader takes this shape.
    if (!bytes || bytes.byteLength < PROBE_CHUNK_HEADER) return null;
    const buffer = bytes.buffer;
    const entry = { offset: bytes.byteOffset, size: bytes.byteLength };
