...
// u_viewProj is NOT declared here: the sun-shadow chunk spliced above already declares it, and a
// shader may declare a uniform once. The merge found that out as
// "ERROR: 0:245: 'u_viewProj' : redefinition" -- the page said so rather than drawing wrongly.
...
//
// THEY MET. This file and features/ssr.js each wrote the closed-form inverse of \`tonemap\`, the
// same function character for character apart from which sampler it fetched, and one of them is
// deleted: \`ws_decode_capture\` in features/ssr.js is the survivor and it takes a colour rather
// than doing its own fetch, because SSR reads a mip and this reads level 0.
vec3 refract_scene_radiance(vec2 uv) {
    return ws_decode_capture(texture(u_behind, uv).rgb);
}
...
export class Refraction {
    constructor(gl) {
        ...
        // THE SAME TWO UNITS THE SHARED CAPTURE IS ON, from the register in web/js/gl.js. When
        // this is sharing that capture — which is the normal case and what §5 of the integration
        // plan settled — these bind the very same texture objects, so there is one target, one
        // pass and one pair of units. When there is no shared capture they hold the fallback copy
        // instead, and SSR's own `u_ssr` is 0 in exactly that case so nothing reads it as a mip.
        this.colourUnit = 13;   // UNIT.captureColour
        this.depthUnit = 14;    // UNIT.captureDepth
