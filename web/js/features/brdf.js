...
// The four arguments after ao are the other branches' terms, handed in rather than recomputed.
// (No back-quotes in here: this is inside a template string and one of them closes it.)
// They are here because every one of them is a MEASURED finding that this lobe would otherwise
// undo:
//   reflected   what the probes and the screen-space march say is along the reflection, which is
//               the room and not the sky -- the whole reason a mirror in a hall is not black
//   envBelief   how much of that to believe. It also fades out the sky-visibility stand-in: left
//               in where a probe already knows what it can see, an interior reflection is
//               darkened twice
//   bounced     the colour irradiance volume, added to the ambient and never replacing it, so the
//               sun and sky bytes are not counted twice
//   baked       the ambient-occlusion atlas, on the ambient and by its square root on the
//               specular, because a recess still sees a sliver of sky in its own reflection
vec3 ws_shade(WsMaterial m, vec3 n, vec3 v,
              vec3 sun, vec3 sunColour, float sunVisible,
              vec3 ambient, float skyVisible, float ao,
              vec3 reflected, float envBelief, vec3 bounced, float baked) {
    ...
    float occluded = mix(0.35, 1.0, ao) * mix(0.25, 1.0, skyVisible) * baked;
    float shade = mix(mix(0.25, 1.0, skyVisible), 1.0, envBelief) * mix(0.4, 1.0, ao) * sqrt(baked);
    ...
    // The sky, diffusely -- and the bounce with it. The bounce is ADDED rather than replacing
    // either term: the two visibility bytes carry the direct sun and the sky, and the volume holds
    // only what has already bounced off something.
    colour += m.albedo * (1.0 - m.metal) * (vec3(1.0) - turnedAway) * (ambient * occluded + bounced);
    ...
    // ...and where a probe or the screen-space march has an actual answer, THAT instead of the
    // prefiltered sky. The prefilter stays the fallback: a probe volume does not cover the open
    // air above a building and does not need to.
    vec3 env = mix(ws_environment(r, alpha, room), reflected, envBelief);
    ...
    if (m.clearcoat > 0.0) {
        ...
        vec3 coatEnv = mix(ws_environment(r, WS_COAT_ROUGH * WS_COAT_ROUGH, room),
                           reflected, envBelief);
