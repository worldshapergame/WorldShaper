...
uniform mat4 u_viewProj;        // the vertex stage's, by name -- one location, already set
...
float ws_view_depth(float window_z) {
...
vec3 ws_capture_radiance(vec2 uv, float lod) {
    vec3 y = pow(textureLod(u_sceneColour, uv, lod).rgb, vec3(2.2));
...
vec3 ws_screen_reflection(vec3 origin, vec3 R, float rough, out float weight) {
    ...
    for (int i = 0; i < WS_SSR_STEPS; ++i) {
        ...
        float scene = ws_view_depth(texture(u_sceneDepth, clamp(screen, 0.0, 1.0)).r);
...
export class Ssr {
    constructor(gl) {
        ...
        // Half the canvas's pixels: a quarter of the work, and a reflection is read through a mip
        // chain anyway. `?ssr=full` puts it back to 1 for looking at what the halving costs.
        this.scale = asked === 'full' ? 1.0 : 0.5;
    ...
    bind(uniforms, camera, clip) {
        ...
        gl.activeTexture(gl.TEXTURE0 + SSR_COLOUR_UNIT);
        gl.bindTexture(gl.TEXTURE_2D, this.target.colour);
        gl.uniform1i(uniforms.u_sceneColour, SSR_COLOUR_UNIT);
        gl.activeTexture(gl.TEXTURE0 + SSR_DEPTH_UNIT);
        gl.bindTexture(gl.TEXTURE_2D, this.target.depth);
        gl.uniform1i(uniforms.u_sceneDepth, SSR_DEPTH_UNIT);
