...
| R10 | ambient occlusion, per face and under it | **THIS ROW WAS STALE and is corrected on 2026-08-20 (D741). R10a, R10b, R10c, R10h and the rest are DONE** and have been since D325-D337 and D381-D396 -- the three rows immediately above this one say so, and so does `22-rewrite-handover.md` section 3. The composite has read measured visibility since D325: `resolve.comp` computes `face_sky_visibility(open_sky, unoccluded) * kSkyAmbient`, and the orientation guess this row was written against survives only as a COMMENT explaining what was replaced. Debug views 17, 18, 19 and 20 are R10h. **A wave of work was very nearly dispatched against this row** -- the agent read the code before writing anything, said so, and built R10d instead. What is genuinely left of R10 is **R10d, the bent normal** (built and measured, NOT shipped -- D741) and R10e-R10g. The composite applies an ambient term with no occlusion in it at all, so the interior is lit as though it stood in the open. Same integral as the sun over a different domain; sub-voxel from a Legendre fit over the face that the existing jitter already pays for; converges once and then costs nothing. §8 R10 | |
...
| R9 | g. the emitter list | **half — the cost is fixed, the persistence is not, and the stage's two named faults turned out to be unreachable.** D587. §8 R9g says a lamp in an unloaded region does not exist and one past the cap blinks out; the facility has **21 emitters against a cap of 1,024** and nothing unloads a chunk from `World`, so neither can happen in the only scene this engine is judged against. What IS real is a third thing this section does not name: `build_light_list` walks every brick of every chunk and runs on **every** announced change — every chisel stroke and every region the ladder pastes. Timed: **13.99 ms a rebuild against the edit's own 0.19 ms**, which is `rebuild_coarse_grids` (D522) four times over, three stages later, in another file. The scan is now per chunk and kept; only what the edit touched is rescanned; the merge still sees every cell, so a fitting straddling a chunk boundary is unaffected. **13.99 → 2.54 ms**, 8 chunks rescanned against 74, the same 21 emitters and the same list version in both arms. Gate is identity rather than plausibility — `light_list_hash` against the whole-world scan on a world built to contain a straddling fitting, 525 tests. `--no-emitter-cache` is the control arm. **What is left is the persistence**, which waits on a world that unloads chunks |
| R9 | f. the fold | **built, measured, and OFF by default — R9 is finished** — D590. A coarse face's sky, near field and bounce are the average of the four faces under it pointing the same way, pulled on a visit it was making anyway: four lookups, one writer, no atomics, which is D191's property kept rather than argued around. Worth **31.0% → 41.8%** of gathering rays landing on a lit face, a third more of what the bounce integrates finding light instead of black. Costs the faces pass **3.66 → 6.43 ms** against a 4.40 budget, so `--face-fold` is opt-in, the same call D586 made for the halo. Not folded: the LAMPS (a child's `pick_light` chose a fitting from the child's position, and four of those are four samples of a different integral), the GRADIENTS (fits over the children's own extents, which do not rescale by averaging), and the SUN COUNTERS (they schedule work — writing them made every coarse face permanently unsettled and took the pass to 7.79 ms). **Three attempts to make it cheaper are recorded with their numbers**, and the third is a fact about where the cost is not: folding one visit in eight cost MORE and won less |
| R9 | g. the persistence | **done** — D588. A chunk's emissive cells are written into the world cache beside the region index and come back with it, so a loaded world knows where its lamps are: the rebuild at load goes **14.2 ms scanning 74 chunks to 0.09 ms scanning none**, with the same 21 emitters and the same list version. An old file carries none, and that is read as *unknown* rather than as *none* — the other way is a building that loads with its lights off. Written by scanning anything not already known rather than by dumping the map, or the file's contents would depend on where the camera stood while it was built, which is R9's own fault arriving through the cache. Format version 3, which costs nothing extra because `src/world` already stamps the cache key |
| R9 | h. the fallback | **measured, and two thirds of it needs nothing** — D589. The sky half is already done: a gathering ray that leaves the world returns `sky_radiance`. The folded-colour half is worth **3 gathering rays of 482,773** on the close camera under continuous editing, which is the most unbuilt geometry this engine can be put in — not built, with the number, because giving those rays a colour needs an irradiance to multiply it by and inventing one is D541–D543's deleted light floor through a third door. **The RULE is the part that was wrong**: *no light path may cause streaming* is stated as absolute with R9a as the single exception, and R9i is a second one that postdates the sentence. A light path may name the one cell that stopped it and the one face it landed on, never what it crossed, and never more than one entry per node per window (D431) — measured at **1 node built over a settled run** against 4 with the rule off |
| R9 | c. the halo | **built, measured, and OFF by default** — D585, D586. The premise had never been measured and half of it was wrong: nothing is MISSING during a pan (the full-sun fallback is nought pixels of 605,945 at every band, because R3e claims a stand-in in the pass that discovers it and R9d reads the coarse face over it). What is wrong is the QUALITY — the leading edge of a turn carries **112 ambient samples against 707** for the identical pixels arrived at from the other side, measured with two arms arriving at one pose from opposite directions so the content is controlled for. The halo widens the primary dispatch by a margin taken from the angle the camera turned this frame, so standing still it is nought and the settled case is untouched by construction; an invocation outside the screen names the face it lands on and stamps it seen, and does nothing else. It works — band 7 goes **14.04 → 18.18** at a 24-frame lead and **21.25** at the cap — and it costs no frame time, because it moves rays earlier rather than creating them. **It costs the sun's stride: 6 in both control runs and 7 in both halo runs**, so every face on screen refreshes 17% less often, which is D527 and D557 a third and fourth time and is invisible in a pass table. A quarter of one edge's deficit is not worth that, so `--halo` is opt-in until a halo face is counted in a class of its own — which is R9b's machinery, not a new idea |
| R9 | the off-screen set | **half, and the off-screen set now carries light rather than merely existing.** R9a, R9b, R9e and R9f are in; R9c (the halo) and R9g–R9h are not. What the bounce integrates and finds black has gone from a third to **just under a quarter**: rays landing on a lit face 18.7% → 29.9%, rays landing on a face in the store with nothing measured 12.4% → **1.7%**, on the close camera settled (D561). The enclosed room's mean pixel goes **127.5 → 150.2** with fireflies 9 → 0, and then **150.1 → 157.4** again when the class stops being capped at a quarter of the table (D570). **The sentence this row used to end with was wrong and is worth keeping**: it said the remaining 21.9% of gathering rays landing on a surface with no face at all was *entirely* R9c's and R9g's. Three dials that already existed take that bucket to **8.7%** without a line of new code (D569), so most of it is claim throughput rather than surfaces nothing ever names. R9c is still worth doing and is about the entry side of a pan; it is not what that number was measuring. Prerequisite for R4c/R4d being worth measuring |
...
| R5 | a. a-trous over the face lattice | **done** — D573, D574, D576. Nothing in this renderer filtered across faces, which is the class the reported fine grid on flat surfaces belongs to (D539 eliminated eight causes and found no ninth because there was not one). A face's `open_sky`, its bounce and its **lamps** are blended with its **coplanar** neighbours' in a 3×3 tent weighted by how well measured each is — and there is no edge-stopping term at all, because a neighbour at the same level and direction one step along a tangent axis is coplanar and contiguous *by construction*, so what a screen-space denoiser spends most of its arithmetic on does not arise. Which three terms was chosen by photographing each on its own first: the lamps read a speckle of 23.0 of 255 enclosed against the near field's 6.0, and the sun is left alone because a shadow edge is a real feature. Two interleaved rounds against `--no-face-denoise`: **roughness 4.3539 → 2.9693** at the steps and **3.0087 → 1.7292** enclosed, **speckle 35.20 → 27.53** and **12.11 → 7.99**, mean pixel moving 0.02–0.19 of 255. Walking out and back: 3.2157 → 2.6859 and 13.210 → 10.847. Flying, the arms interleave. It reads words 0–11 and writes 12–18, so a filter that filters its own output is unrepresentable rather than avoided, and only the composite reads them — a gathering ray takes the raw bounce, or the blur compounds through the light transport instead of through the buffer. Costs **3.45 → 4.06 ms** settled at the steps against a 4.40 budget, and **29.6 MB**. **b's WRITE half is built and OFF (D660) and its READ half is built and ON (D665) — a face short of four shadow rays is drawn part way towards the coarse face over it, so the speckle on anything just revealed is a quarter of the step it was; c's FIRST HALF is in (D664) — the level dither still picks the cell and no longer picks the colour; and d's FIRST HALF is in (D663)** — a coarse node whose per-direction coverage is not full now casts R4d's second march past itself and the composite draws the two surfaces in proportion, so a distant railing stops being a solid stair-stepped bar. One bit of the `in_behind` word tells the composite which of the two blends it is, because a pane scales the near surface's diffuse alone and a partly covered pixel scales all of it |
| R6 | the light meter | **done** — D577, D578. `kPreviewExposure` was 3.2 with no writer since R3d deleted the tracer and R1e the buffer under it, and it is the reason two clips written to test exposure could not be used: `many_lamps.clip` read **248.9 of 255** with 99.9% of the frame over 200, and `exposure_range.clip` read **35.8**. They read **150.6** and **149.3** now. The facility moves 2–6%, not 35%, because `kExposureBias` (+1.3 stops) is a separate constant from `kMiddleGrey` — measured from what the three cameras need to stay put, so what is left is the meter working rather than a brightness edit. It does not pump: two consecutive static frames read 2.991× and 2.991×. `--validation` caught the one fault, a missing TRANSFER_SRC on a buffer that copies into itself, and the picture looked right without it |
| R4 | a. the variable payload | **half — the material half is in, the payload is not.** D582, D583. The user chose this stage over R9c. A face is *(node, level, direction)* and the light pass reads the marcher's folded average colour for an albedo, because it has never had the interned tables bound — so nothing anywhere knew whether the surface under a face was limestone or gilt, and a lobe is roughness and metalness or it is nothing. The two tables are on the node set now (22, 23) and a face resolves its own material **once**, by descending to its own brick, into a card-owned word a slot: `GpuFace::bins` is the host's side of a record with two owners, so it stays nought (D295, D528). Costs nothing measurable in either case, two flags of one build interleaved — close settled **4.013 / 4.037 against 4.061 / 3.994 ms**, flying **7.867 / 7.433 against 7.387 / 7.032** — with equal sun samples and equal seen counts in both arms, which is what says the arms did the same work (trap 20). **The census is the number that sizes the rest of the stage**: of the faces a pixel is reading, **14,130 of 111,373 carry metal enclosed, 22,158 of 416,143 close, 7,479 of 214,983 flying**, so the payload serves tens of thousands and not hundreds. Reported as a distribution and never as a count past a cutoff, because there is no roughness threshold in this stage and an instrument that picks one is how a cutoff reaches the shader later. **Limit**: only a level 0 face has a material; a coarse face stands over up to 512 voxels that need not agree and answers NONE, which is R9f's fold when somebody wants a distant dome to look like metal. `--no-face-materials` is the control arm and `--debug-mode 21` is the view |
| R4 | c. outgoing bins | **DONE — the IMAGE is there, D703.** The bins stopped covering the hemisphere and cover a CAP about the direction the eye is in, because the composite reads exactly ONE direction out of a face (D597) and a face is one voxel, so every pixel on it looks from within 0.86 degrees. `cap = clamp(0.5 * alpha * side, 6deg, 57deg)` — continuous in roughness, no threshold. Chrome gets **2.0 degree bins instead of 13.5** for the same 36 bins, the same pool and the same 864 rays. The chrome sphere carries the sky gradient, the floor slab edge and the white floor where it was a uniform pale ball: **3.838 of 255 over 65,753 pixels against a run-to-run floor of 1.136**, at +0.4%. And the burst follows the bin count now, so both classes converge in the same ~108 visits. The older note: the storage and the split are in; the reflected IMAGE is not. D591, D592. The composite splits what leaves a surface by metalness read as a quantity: the diffuse loses the metal's share (in `face_terms.glsl`, so the gathering ray applies it too — D420), the sun comes back through a GGX lobe with **no storage at all** because the face has already measured what fraction of the disc it can see, and the environment comes back out of **sixteen outgoing bins** filled by the gathering ray the face was already casting. The bins are a **pool of 65,536 blocks (8.7 MB) that faces HOLD** — four-way on the slot, cold at 600 frames, weakest loses — because 34 words × 1,081,344 slots is 147 MB for a number that is nought on nine faces in ten, which is §4's *matte stone allocates no payload*. **Costs nothing measurable**, two flags of one build interleaved: enclosed faces **3.228 against 3.242 / 3.200**, close **3.766 against 3.924 / 3.879**, resolve 0.869 / 0.864 / 0.844, flying and chiselling both inside their own spread. The picture moves **4.467 of 255 over 119,491 pixels enclosed, 8.778 over 251,607 at the great door**, and is STEADIER not noisier — two consecutive settled frames differ by 1.95 against 2.32 with the stage off. **What it does not do is draw a reflection you can recognise, and D592 is why**: sixteen bins is 20.4 degrees, this building's metals are genuinely rough, and the near-mirrors are dielectrics ranked below the pool's floor. 256 bins were built, measured and reverted — **the bin count is bounded by the RAY budget** (512 far samples over 256 bins is two apiece) **and a cosine-weighted ray cannot fill a grazing bin**, which is where a reflection is read from. `--no-face-lobe` is the control arm, `--lobe-floor` the capacity dial, `--debug-mode 22` the residency view and `23` the term on its own **Open, and it blocks every card-free picture of a scene with glass, water or metal in it (D656)**: the lobe pool is what makes a software-rasteriser run die at frame 9. `--no-face-lobe` reaches frame 120 and `--no-lobe-ray` does not, so it is the claim, the bins or the probe rather than R4b's march; the validation layer says nothing and forcing `robustBufferAccess` on does not move it. |
| R4 | b. bins from coverage | **DONE — the ray (D594) and now the RULE (D694).** The bin count is `min(what the roughness wants, the pixels on the face)`, quantised by the pool's own block and by nothing else — no roughness threshold and no coverage threshold. It lives in `face_worklist.comp`, the one whole-store walk that is not the shading pass, so no reader changed. The distance is taken to the face's COARSE PATCH, which is D599's fault fixed: 512 fine faces then agree by construction, so the boundary is a step the cross-face blend can pair across instead of a per-face speckle. Mirror clip, same content hash both arms: 2,695 of 30,565 faces take the sharp class, payload 11,749,600 B against 9,709,456 (+21% for 8.8% of faces), declines unmoved. **The IMAGE is still not there and that is now measured four times** — `kLobeBigSamples` is 24, and twenty-four samples of a radiance is twenty-four however many bins there are (D592, D594, D599, D694). The lever is the ray budget in `shade_faces.comp`. The old note: D594. A face that holds a lobe casts its own ray: it picks a bin round-robin, draws a half vector from that bin's kernel, reflects the bin's direction about it and marches unbounded — Karis's split-sum prefilter with the view direction taken as the BIN rather than as the normal, which is the whole of what fills the grazing bins a reflection is read out of. Bins went 16 → **36** and the pool 65,536 × 34 words → **131,072 × 76 (38.9 MB)**, because the worth floor now admits the glass and the water and that takes the population asking to 44,700 — declines **47% → 0.8%**. Worth **3.913 of 255 over 103,874 pixels** at the great door against `--no-lobe-ray`. Costs **nothing measurable settled** (close faces 4.229 against 4.218) and **1.5 ms flying** (9.318 against 7.825). Three tunings, each measured: the burst is **8 rays a visit and not 32**, which reverses D394 for this term because a lobe ray is unbounded where an ambient near ray is not (11.9 → 9.3 ms flying); a warm block is only taken by a face worth **1.5×** its holder, because a take-over resets the burst and equal-worth faces traded one for ever (417 hand-overs a frame → 13); and the far ray's splat is kept only in the control arm. **R4b's second half is built, measured and OFF** (D599): a second size class of 144 bins as four consecutive blocks. **Coverage turns out not to be the lever** -- a face is one voxel and the pixels on it span 0.86 degrees, so sharpness is the bin and coverage does not enter -- and gating on it made the class patchy across one surface, which is D387's discontinuity in a size class. Driven from roughness instead it is affordable in MEMORY (17,389 of 21,200 holders qualify, nought declined) and not in RAYS: speckled at 8 samples a bin and still speckled at 24, which is four times the rays for the same noise. `--lobe-coverage` is opt-in. **What is left**: the mottle at 24 samples a bin, which **R5 closed** (D595): the lobe is blended with its coplanar neighbours' bin against the same bin, ONCE, on the visit after it converges — speckle at the great door **21.73 → 14.75** on `--debug-mode 23`, at 4.292 ms against 4.229 settled |
| R4 | d. incoming bins | **transmission and refraction are both in, and the refraction has now been LOOKED AT (D656).** D601–D604. A face resolves what it lets through (D601); the shadow, ambient, gathering and lamp rays stop being blocked by a pane, which is the largest single number this stage has produced — a wing hall lit only by lamps because every window in the building was treated as stone comes back daylit, **95.577 of 255 over 1,018,413 pixels of 1,024,000** (D602). The tint the marcher accumulated was read by nothing for one commit, and the attenuation was per VOXEL where opacity is a property of the material: taken per METRE and rooted to the voxel, a four-voxel pane transmits 0.987 instead of 0.32, and the light meter caught it before the eye did (D603). Then the PRIMARY ray: a second `node_march` per glass pixel into a new `rgba32ui` image on binding 26, shaded down the same 533-line path as the pane by extracting it into `shade_surface`, and composited as `(1−T)·diffuse + specular + emission + T·behind` — a window is fifteen glazed lights with the wooden bars across them where it was a flat milky panel, **19.85 of 255 over 707,823 pixels**, costing **5.147 → 5.393 ms** at a camera facing one and nothing measurable at a camera without a pane on screen, all of it in the visibility pass, 0.737 → 0.966 (D604). **What is left**: `ior` is carried by the material and used by no ray, so nothing is displaced; no Beer-Lambert over the true path; no dispersion **The stale sentence this row ended with — *`ior` is carried by the material and used by no ray, so nothing is displaced; no Beer-Lambert over the true path* — was answered by D652**: the primary ray bends at both interfaces, absorption is `exp(-absorb·metres)` over the true path, and total internal reflection reflects. **And the picture, owed since D652, is taken (D656)**: card-free on `clips/refraction_small.clip` at 640×400 settled to frame 120, both arms hashing `0fcdedc147df45a4` — the seam where the red wall meets the blue one, and the wall's bottom edge, step sideways where a pane covers them and run straight where it does not, and the pane's own top edge cuts a step into the wall's top line. **Shot with `--no-face-lobe`**, because R4c's lobe pool is what kills a software-rasteriser run at frame 9 (D656) — so refraction and directional reflection in ONE frame, which is what a window actually is, is still unseen, and so is the cost of either. Dispersion is still not built. |
| **R7** | **a and b DONE — D698** | The beam pre-pass, carried in `04-rendering.md` §1 since Stage 3 and never built. One coarse ray per 8x8 tile CORNER at sixteen pixel widths; the tile takes the **smallest** of its four. Four corners is enough because a cell at least as wide as the tile that overlaps it must contain one of them. `occlude_unknown` is ON — an unbuilt cell must be MATTER to the beam, or it starts a ray past a building that has not streamed and the building is a hole. R7b never decides: **`start = min(beam, temporal)`**, so a stale reprojection can only cost time and never a surface. **Primary visibility at 4K, arm to arm: 1.18 ms against 16.81 outdoors (14.0x), 1.96 against 15.07 close, 0.89 against 4.65 mid, 2.32 against 12.49 sky — and 0.76 against 0.56 at distant, where the ray was already trivial and the beam is a net cost.** The stage's own ratio gate (4K within 1.6x of 1440p) is met at close and mid only, and that ratio is the weakest number here — it divides runs taken minutes apart on a contended machine. **Two things open: the enclosed camera at 4K has no matched pair, and the FACES pass moves 7.4 → 13.4 ms there. The cause offered for it is refuted by the beam's own code — it passes `report`, `report_used` and `report_face` all false, so it names nothing to residency.** **R7c is NOT done** — step count in the descent, in `node.glsl`. | 
| **R6** | **a and b DONE, c was already done at R3d — D702** | Post is its own pass: `bloom_down.comp`, `bloom_up.comp`, `post.comp` and `src/gpu/post_pass.*`, with `pt_post.glsl` rebuilt as a header that declares no binding. The 437-tap gather is gone. **0.706 ms at 4K on the fixed grid — the gate is met — and 1.113 ms with the camera turning, which is over it**, and the remainder is priced at 0.374 ms of pure copy. 0.109/0.327/0.706 across three resolutions against pixel ratios 1:3.6:8.1. **The glare is still added AFTER the curve**, which is the one thing bloom is not for; the patch for `resolve.comp` is written and was handed forward. **R6c: nothing left in the code, but five HARNESSES were passing `--pathtrace` and one did it unconditionally** — an unknown argument is warned about and ignored, so every face-count run since R3d measured the real-time path and said tracer. Fixed. | 
| R8a, R8e | | not started. **R8 is re-sized to L**: R8c and R8d are moved to R11, which needs both for the whole world rather than for a mode |
| **R11** | **the world source, driven by pixels** | **f IS NOW DONE TOO — D692, and all three of its ways to lose a building were OUTSIDE the format.** The cache could carry named edit boxes from the day the mode landed and nothing in the repository ever produced one; `edit_boxes_from_ops` is the bridge and it takes EVERY op including the ones that changed nothing, because a swing that met only air is exactly the case a difference cannot see. Two more under it, both the shape where nothing fails: a named box in a chunk NEITHER world has anything in was dropped (a chisel in an empty field reached no chunk and was written nowhere — the guarantee failing precisely where it was written for), and `adopt` replaced the type table while the baseline was a world the READING run interned in its own order, so 478,720 of a 512,000-voxel wall came back in somebody else's materials and the reader BLAMED THE CLIP. Estate at metre 4: voxel payload 1,068,665 → **1,448 B, 738x**; read back in 155 ms against 776,979 ms to sample it. `--clip-plus-edits` is additive and does NOT lift the refusal to cache an edited world as the clip's own — that refusal is about what a clip's cache IS, and making the loaded file a difference is a decision for the user. a, b, c and d are DONE — d SHIPPED ON at D673, and the loading bar is gone: 297 ms to playable on the estate against a control arm abandoned at 80% of 261 M voxels. It had been built for weeks and gated on TWO flags (`!no_coarse_paste || stipple_at_coarse`) while every heading naming it named one; g's gate turned out to need no work; h passes its near case; e and f are not started.** **D649 — the ladder CAN carry the stipple verdict exactly, and the skirt it needs doubles the sampling.** `--stipple-tiled` tiles a clip into the node boxes the ladder samples and counts them three ways against one whole-clip reference: **bare** (what ships) is 50 materials DIFFER, **inner** (free, the node's own sample counted one voxel in) is **2 DIFFER and both are dithers, −455 −554**, and **skirted is EXACT — 81 of 81 materials, surface and specks off by zero, 0 DIFFER**, reproducing the shipped `27 358 392 455 509 554` material for material. So D629's method holds at the ladder's own geometry, which is what R11d needed. **The price refutes D647's step**: a skirted node is **6.69 ms against 3.31** (2.02× at metre 8, 2.13× at the authored metre 32), and doubling the ladder's 6,322 ms to remove a 2,754 ms bar is three and a half seconds the wrong way, paid on the nodes the player is waiting for. What survives is D628's own surviving route, still unbuilt: **read the skirt out of the WORLD after the paste**, where a neighbour costs a memory read and not a field evaluation. **D648 — the tree builds off Windows**, so all of that was measured on a machine with no card. **D635 — R11h's near case: a chisel cuts properly with no coarse build under it.** A 20 m cube carved through the middle of the facility at frame 400, both arms: pasted **125,420,017 → 62,608,814** voxels, `--no-coarse-paste` **19,751,324 → 25,232,463**. **The count going UP is the carve working** — a twenty-metre hole opens the building to the sky, so what was occluded becomes visible and the ladder builds it; the photograph is what settles it, and the cube is cut with clean sharp faces and the portico revealed behind it (six columns, pediment, entablature) is at full detail. **And a counter that reads alarming and is not**: the pool logs `0 leaves rebuilt` against the pasted arm's 1,741, because `leaves rebuilt` counts leaves the pool ALREADY HELD and had to re-derive, and this arm held far fewer — a counter about what the POOL held is not a counter about what the EDIT did, and reading it as one would have condemned a working feature (trap 7's shape again). **Not tested: the far chisel**, sixty metres into a surface never approached, where the proximity radius must hold *sampling* rather than *residency* (R2c, D199). **D634 — a partial world resumes from another camera, and accumulates across launches.** The one measurement D632 asked for, cache cleared, three launches: A (enclosed, cold) ends at 50 chunks / 19,751,324 voxels in 18.6 s; B (close) **loads A's world in 52 ms** and ends at 70 chunks / **23,137,962**; C (enclosed again) loads B's in 72 ms and costs **9.1 s against 18.6**, sampling **1,589 ms against 7,585**. The world is what has been looked at, it grows as the player moves, it survives the session and it converges across launches. **D633 — R11g's gate is already satisfied, and needed no work.** D632 named it the blocker: a pixel-driven world has a camera-dependent hash by construction, so the fear was every baseline row refusing itself. Measured: the enclosed camera gives **`70b51a3f94dc44ba`, 19,751,324 voxels, twice to the digit**, and the close camera gives `42af22ca9c2820a5`, 3,631,394 — the same camera reproduces exactly and a different one does not, which is precisely what the gate needs. `baseline.ps1` finds its row by (mode, view, size) and **never compares across cameras**, so it never had to change. `--settle` generalises exactly as §5 hoped, and D627's stand-down is what makes it a fixed point rather than a timeout. Two things this does not say: a CSV taken with the paste and one without are not comparable row for row, and the gate will say so correctly. **D632 — D631 read the design goal as a bug.** D631 called the smaller world *"a sixth of a building"*; that reading is wrong, because a world holding only what the camera could see is §1's third ask in the user's own words. The test D631 never ran is the camera moving: standing still 19,751,324 voxels and 1 stand-down, **walking `--fly 0,0,3,25` 29,622,982 and 6 stand-downs woken 6 times by the camera**. **A completeness figure measured from one camera cannot tell a pixel-driven system from a broken one** — *"stopped early"* and *"finished what it could see"* produce identical instrument output, and only movement separates them. Trap 8 arriving at the thing trap 8 exists to protect. **D631 — R11d's first form, and the trap it found.** The paste goes and the sample stays: `paste 0 ms, compact 0 ms`, ready at **t+3,026 ms**, nodes seeded `applied_per_metre = 0` because with nothing pasted the world holds nothing. The finding that mattered: **the picker refuses a node it cannot see by casting a ray through the WORLD**, and with nothing under it that ray is asked of a world still being built — early nodes are refused by nothing, the geometry that happens to arrive first occludes everything behind it, and a refusal outlives what caused it. Occluded **6,022 → 8,446**. D632 corrects the conclusion but not this mechanism: the honest statement of the risk is that the ladder can reach a fixed point of its own BUILD ORDER rather than of the building, and every counter reads calm while it does — `neither` 0, nothing deferred, nothing starved. **D630 — option 2 is built and ships OFF, behind `--stipple-from-world`.** D629 put three routes to the user and they chose taking the verdict at the authored resolution. Two measurements rather than an opinion keep it off: cleaning the world is **19.0 → 40.3 s** (68 chunks of 258³, 1.17 G cells walked twice, serial), and **a partial world protects 0 materials against a whole one's 1**, so the verdict would depend on where the player walked. **A whole-building verdict needs a whole-building measurement, and the only affordable one is the up-front sample** — which is what R11d wanted to delete. **D629 — route 1b works, and it proves the verdict is a property of the RESOLUTION, not the building.** Counting against the world with a one-voxel skirt reproduces the metre-32 whole-clip verdict exactly, `{27}` for `{27}`; the disagreement that remains is the finding — **metre 8 protects six materials and metre 32 protects one, on the same building**. So the shipped verdict is metre-8 by accident (it is whatever `--clip-coarse` samples at, and nothing in the world is ever built at metre 8), and **the specks being judged are metre-32 while the judge is metre-8**, shipping since D610. **D628 — route 1 refuted by measurement.** Summing the ladder's per-node speck counts is additive in arithmetic and wrong in fact: `paint_specks` reads outside its clip as air and **57.8% of a leaf node's cells are on its own boundary** (8³ − 6³ of 8³), so **11 of 35 materials change verdict and two of the six deliberate dithers are destroyed**. `sampler.clip` agrees perfectly, 0 differ, while the facility disagrees on eleven — **that clip is not the gate for this**. The comparison instrument is kept as the gate any future route is judged by. **D627 — the ladder never stood down, so a finished world swept for ever and wrote half a gigabyte.** D626's 4,788 ms of a cached load delivering nothing, reproduced and then found to carry a second cost nobody had counted: a sweep that delivers nothing still marks nodes `done`, so `save_refined_world`'s *"has anything been sharpened"* guard passed every time and **the whole cache was rewritten 28 times, 504 MB to disk, on a run that sharpened nothing at all**. The ladder now stands down when a sweep finds nothing and is re-armed by the only two things that can change the answer — the camera moving (`kStandDownMove` a brick, `kStandDownTurn` two degrees) and `announce_world_change`. **Cached launch: wakes 270 → 33, pick 584 → 147 ms on the main thread, entries swept 17,876,700 → 2,184,930, occlusion rays 99,600 → 11,832, cache writes 28 (504 MB) → 1 (18 MB), wall 6.90 → 6.04 s, world identical both runs.** The **cold load is byte-identical at `789c8a80f40323a1`** with the same 32,712 / 23,356 nodes and cheaper anyway — wakes 486 → 249, pick 814 → 519 ms — because the sweeps it skips are the ones that would have found nothing. **Two versions of it were wrong and both are worth knowing.** Standing down on the FIRST empty sweep loses the tail: an occlusion refusal is provisional and retried every `kRefuseFor` wakes, and a sleeping ladder never wakes, so the cold facility settled **32 nodes short** and its hash moved to `46ae5ba4f78bad20`; the stand-down therefore waits a whole `kRefuseFor` window of empty sweeps, which means every memo has been retried. And *"stand down once the clock passes the furthest-out memo"* **can never be satisfied**, because every sweep rewrites the memos it refuses, so the horizon moves with the clock — the window is measured from the last sweep that **delivered**. **The method note cost most of that session**: the stand-down counter read *2 stand-downs on a cold load*, which was read as *"two events cannot have moved the hash"* — but the count was low BECAUSE of the skip under suspicion, so the instrument was measuring the fix rather than the fault. `git stash` + rebuild + the same command settled it in one run. **A counter taken from inside the change is not a control; reach for the control arm before the third theory.** **D626 — a cached world was frozen at sixteen voxels a metre, and the log line said the opposite.** `save_refined_world` wrote only leaves already at the authored resolution, and from the default camera **no leaf ever reaches it**, so the file was written with an **empty** region list — which the format defines as *"this world was not built through the ladder"*, i.e. a one-pass authored world with nothing to do. The loader duly did nothing, on every launch after the first, for ever: **3.83 s** and `no ladder, the world is at the detail the clip asked for` over a building permanently blockier than its clip. That is trap 7 inside the file format — *"nobody sharpened anything"* and *"there was never anything to sharpen"* were the same empty list. D625's parting finding (`already_sharp` tests seeds for containment in a smaller box, so 19,680 saved boxes mark nothing and 11.7 s is spent re-sharpening) is real but needs a camera that reaches authored detail; **do not quote its 11.7 s for this one**, which is the second time a figure has been carried across cameras and cost a session. A third fault the same change closes is latent and worse than either: the old resume reset every node's `applied_per_metre` to the coarse floor, and a paste REPLACES, so a resuming run could drop a 16/m answer on top of 32/m geometry — the building getting blockier the more it is loaded. **The fix**: `CachedRegion` becomes the ladder's WHOLE leaf set carrying `key[3]`, `level`, corners, `applied_per_metre` and `done`, and `resume_refinement` rebuilds `refine_regions_` through `refine_node_of` on each saved key — a pure function of this run's own bounds, so a stale file cannot inject a box the tree does not own — instead of seeding at level 8 and testing containment. Format **4 → 5** (first run after any build is cold). Rejected with reason: giving reconstructed ancestors `applied_per_metre = min` over their subtree breaks the invariant that the field is uniform over a node, and the picker would sample such an ancestor coarsely and paste it over sharper children. **Measured**, same camera and flags: cold hash unchanged at `4f6ac8651fc45587`, which is the gate; the cached arm converges over three launches `b41559abbd025759 → fa627ed48d5d488c → fa627ed48d5d488c` and then reproduces byte-identically, level-3 leaves **0 → 62,752**, solid voxels 125,396,992 → **125,362,512** (a coarse sample over-fills at the surface, so sharpening removes matter). **The honest cost: the steady-state cached load is 3.83 → 5.81 s.** Committed anyway, because the old arm is fast by being finished and wrong, and there is no correct cached arm in the old build to compare against. **The 2 s is recoverable and the number is on record**: 4,788 ms of the new 5.81 s is ladder cost delivering **0 nodes** — 300 wakes, 149 sweeps, 22.4 M entries, 123,975 occlusion rays, **619 ms on the MAIN THREAD** — because 13,409 leaves are permanently occluded from this camera, `done == refine_regions_.size()` never becomes true and the teardown never fires. A *"a whole sweep found no candidate → stand down until the camera moves"* rule is the next small thing and turns this into a strict win. **D625 — the despeckler was off on every cached load, and a cached world was a DIFFERENT world.** The stipple verdict — which materials are a deliberate dither and must be left alone — is taken once over the whole clip in the up-front coarse build, and `resume_refinement` never took one, so a cached load began with an empty verdict and `forge::despeckle` reads empty as *"leave every speck alone, everywhere"*: the despeckler ran 23,000 times a load and did nothing, on the path every launch after the first takes. Found by giving the per-node `DespeckleReport` a counter instead of throwing it away — cold reads `35 materials, 512 repainted, 663 left`, cached read **0, 0, 0**. A cheap re-derivation was measured and refused: at sample metres 8/4/2/1 the verdict sees 35/31/26/19 materials and metre 4 shares two of metre 8's six protected ones, so a coarse stand-in is a different verdict and not a cheaper one. The fix carries it in the world cache, format **3 → 4** (every existing `.world` is therefore rejected and rebuilt), with `stipple_taken` kept separate from the list because *"no material is a dither"* and *"nobody asked"* are both an empty map — trap 7 exactly — and a file without one now warns by name. **The result is the content hash**: the cached arm moved `007113c0915ed6b1 → 789c8a80f40323a1`, which is the cold build's, twice running. **And the resume log now prints the ratio that names the next big one: `0 of 120 nodes sharpened from 19680 saved boxes`** — `already_sharp` tests seeds for containment in a saved box and since R11c a box is smaller than a seed, so nineteen thousand paid-for boxes mark nothing, and a cached load re-sharpens 23,324 nodes and 11.7 s reproducing voxels it has already loaded. The region list is currently used for nothing but whether it is empty. **D624 — every load since D623 threw its last batch away, 606 voxels of the gate clip.** `enlist` marks a node `done` when it is PICKED and not when it lands — it must, or the next pick samples it twice — so `left == 0` in `deliver_refinement` reads true with a batch still out at the sampler, and the pick sits directly above that count, so an outstanding batch there is the COMMON case rather than a corner. The ladder declared itself finished over work it had already paid to sample and tore the worker down under it. D622 got away with the same test because its teardown never called `stop_refine_worker()`, so the dropped batch landed anyway into a reset script and plan — a use-after-free that happened to preserve the count, and the reason the old build printed *"fully sharpened"* TWICE. One condition: `left == 0 && !refine_busy()`. Gate back to **1,430,104 voxels, `a1f8bc6c656343b7`** over three runs settling at frames 311/300/304; facility unchanged at `789c8a80f40323a1`, wall 17.6–18.2 s against D622's 17.3, which is spread and not cost because that camera settles at 32,712 of 40,436 nodes sharpened and never reaches the fixed point at all. **Which is the method finding: D623 read the facility hash as its control arm and called the gate restored, and the facility hash could not have moved whatever the change did — check the arm that can FAIL.** **D622 - the LOAD is 4.8x shorter, and it was three waits rather than a cost.** Nothing had ever measured where a load's seconds go; a run-total line beside the settle line named all three in one run. **The batch was sampled one node at a time on one core** - the loop was serial and each `forge::sample` was handed the pool to split ITSELF across, which R11a had already measured as worth nothing (1.389 threaded against 1.391 serial; a node is eight z slabs), so 33,912 ms became **7,691** by giving each worker a node. **7.1 M occlusion raycasts to choose sixteen nodes** - two full sweeps of a 40,436-entry list, and the first fired a ray for every node beating the front runner, but a node the ray REFUSES never becomes the front runner, so `keenest` never rose and all 6,042 permanently occluded nodes fired on nearly every wake; one cheap sweep with no ray in it took **25,996 to 1,736 ms and 7,103,492 rays to 149,512**. **The ladder slept four fifths of the load** - batch 16 was sized in D617 for a serial sampler, so a wake cost 4 ms of a 22 ms frame and `pump_refinement` runs once a frame: 1,730 wakes over 1,716 frames. Batch **16 to 128**. Wall clock **83.6 s to 17.3**, frames to settle **3,268 to 701**, and the frame DURING the load **24.1 to 17.7 ms**. The gate is not the timing: `sampler.clip --refine-all --no-despeckle` returns **`a1f8bc6c656343b7`, 1,430,104 voxels**, byte for byte D615/D616's reference, so all three are scheduling. Measured and NOT carried: more than half the machine for the sampler takes sampling 6,361 to 4,509 ms and the paste **140 to 1,607** - D511-D514 from the other side. **The honest ceiling**: the ladder asks the field for 3.8 M voxels against 139 rules at ~8 us each, 32 core-seconds, so five workers are a **6.4 s floor** and 51% of what remains; a double-buffered pick is worth ~12.4 to ~8 s and the up-front coarse build is R11d's 3.6 s. Sub-second means not evaluating the field per voxel on a CPU, which is **R12** - and this is the first hard number for why R12 exists. **D621 — the "huge brick blocks" are TWO faults and one of them is closed.** D620 bounded it with one sentence from the player — *the aim circle does not detect the lumps* — and the aim circle is a CPU raycast against `World`, so the lumps are not voxels at all. **The ones that survive a settle were made by `paste_clip`**: a refinement paste REPLACES, so it erases the coarse voxels a finer sample supersedes, and a write through `brick_for_write().assign()` that takes the last voxel out of a brick left it **allocated** — which `world_has` reads as matter, so the child mask is set, `build_leaf` correctly refuses to build a node with nothing in it, and the cell stays unbuilt-but-occupied for ever. `drop_brick_if_empty`'s own header has said since D357 that a bulk writer has to say so and `op.cpp` was taught to; the paste never was. Settled, enclosed camera: **304 allocated empty bricks → 0**, sun faces shadowed by a cell the pool has not built **12,517 → 113**, gathering rays stopped by one **8.2% → 0.0%**, fireflies **108 → 0**, 20.2% of pixels changed, GPU 7.86 → 7.66 ms and pool CPU **0.91 → 0.29 ms**, with the world byte-identical (`27a59697104af7c9`, 125,419,666 voxels). `--no-paste-drop` is the control arm. **Two method findings, both now traps 26 and 27**: all three of the engine's render-tree audits agreed perfectly with 304 lumps on screen, because every one of them is downstream of `world_has` and `world_has` was the liar; and `out_of_memory` was set at one of the pool's four allocation choke points, so a jammed pool printed `deferred 0` and counted every refused build as `built`. **The ones a player watches DURING a load are still open and are not this** — both arms are the same picture at frame 600, where nought empty bricks exist. Read with the new counter: the pool is pinned at its **262,144-leaf ceiling** for the whole load, refusing **252–338 builds a frame**, evicting nothing, while the settled world needs **33,282**. Why the load's peak demand is eight times the settled demand is undiagnosed and is the next question. **D619 — the ladder was starved, and that is what the "huge brick blocks" were.** The report was blamed on the up-front coarse build twice and it was neither time: a census printed beside the settle line asks, of every node left coarse, which of the picker's three tests refused it, and **721** of them were refused by none — they simply never got into a batch. The three tests have different tenures, and the odd one out is the occlusion ray: it must not mark a node `done`, because the camera will move, so the big near nodes behind a wall came back every frame, ranked highest every frame, and filled a shortlist of sixty-four for ever. The batch of sixteen delivered **1.22 nodes**, and exactly **1.00** by the end. Shortlist **64 → 512** with the facing demotion folded into the cheap sweep, and an occlusion refusal **remembered for 32 wakes** rather than rediscovered: batch **1.22 → 15.26**, sharpened **10,486 → 32,680**, "neither" **721 → 0**, solid voxels moved 0.06%, and the urns have lids and handles again. Two traps it exposed: **`--settle` was never settling** — it hit its 180-second give-up on every run, which voids D618's one-flag reading (two timeouts, not two fixed points) — and a GPU mean over an unsettled 5,849-frame run is not comparable to one over 30 settled frames, so **30.350 against 6.801 ms is not a speed-up**. R11d is unblocked and still owed. **R11c** (D616): a node is sampled at `256 / 2^level` capped at the authored resolution, and the split threshold became eight voxels at a pixel each rather than a quarter of the distance -- with the resolution following the level, a quarter is voxels sixteen pixels wide. Forced to full detail on `clips/sampler.clip` the level ladder builds the **same byte-identical world** through 9,819 samples at six resolutions (`a1f8bc6c656343b7`). Facility: 120 eight-metre seeds become 30,898 nodes, 9,272 sampled, 14,096 left coarse behind walls, **worst paste 12 ms** against a 16 ms gate. The fly-in gate is not measured. D611, D613, D614, D615. **R11b**: the eighteen twelve-metre boxes are gone and the unit is a node of the render tree, seeded at four metres and split to one as the camera approaches — on `clips/sampler.clip` the node ladder and the box ladder build **byte-identical worlds** (`a1f8bc6c656343b7`), with despeckling's edge voxels the only residual (46 cells of 152,064 on the facility). The facility A/B is not run. A one-voxel skirt was built and reverted: it lost 240 voxels of 1.43 M because the sampler answers differently for a box one voxel larger, which the instrument now measures. **R11a, the instrument**: `--sample-cost` and `tools/samplecost.ps1` sample one node at a time at every level a node can be asked at and compare it against the same node read out of the biggest box that fits. One node at the leaf is **1.389 ms** against **0.213** for one that holds nothing, the whole facility node by node at the leaf is **374 core-seconds**, and asking one node at a time costs **21.5×** what the same box costs in one call. Two findings the later sub-steps turn on: **the fixed cost is the paint RULES and not the box** — an empty node is 0.213 ms on the facility's 139 rules and 0.012 ms on a clip with four, so 85% of what a node pays is `sample()` re-deriving every rule's slack and box on every call — and **the pool is not worth waking for one node**, 1.389 ms threaded against 1.391 serial. The agreement check **failed and found a fifteen-month-old fault**: a box was settled empty in bulk without allowing for the thin-feature rescue's own half-cell-diagonal reach, which is 2.7 cm at metre 32 and 87 cm at metre 1, so the same node disagreed with itself at levels 7 and 8 — 108 cells over 64 nodes. Fixed, costing ~3% on a region at authored detail and changing no matter at all at the resolutions the game uses today. Baseline in `documentation/baselines/r11a-sample-cost.csv`. R0–R10 rewrote everything to the right of `World` and nothing to the left of it: a clip still becomes voxels over a fixed list of eighteen boxes at one of two fixed resolutions, both chosen before the first frame is drawn. §1's rule is therefore true of the marcher and false of the thing that feeds it, and a player sees that as a loading bar, a blocky first minute and detail arriving in slabs. Reproduced on the shelf world with no usable cache: coarse build at metre 8, **ready at t+3,615 ms**, eighteen boxes at 624–7,406 ms of sampling each, **8 of 18 done by frame 900**. The mechanism has been R8c since 2026-08-09 and the row below has connected it to loading since 2026-08-11 — what was missing is that R8c sat in §7, an experimental mode off by default and last in the order, so this plan could be followed to the letter and still leave the loading bar. **a is the instrument and nothing may be sized without it; b and c are what a player sees; d is the headline and needs both under it; f is last because it is the only one that can lose data** |
| **R12** | **the field on the card** | **a and b are DONE, and BOTH named speed steps now are too — D690 the compiler, D691 the descent. The card is no longer the slow arm.** D691: `Field::eval` culls a union three ways and the card had ONE. Nearest-box-first (D638's own finding, made on the CPU) and the carve cull were both missing, so a point in the seventh building of the estate evaluated the other six in full before it held a number small enough to reject anything with. A cell walks **2,876 nodes against 8,273**, the control arm reproducing D681's recorded 8,231; on `sampler.clip`, where both arms did identical work, **33 a cell against 67**. The gate is 0 differing cells in matter, material and mask on both clips and both arms, and refusals are now printed WITH their denominator (0 of 16.1 M) because a silent warning and a run that never asked make the same silence — D676 from the other side. The bounds are deliberately still the CPU's unsound ones: D646 measured the sound repair at 45x, and a card that can reject a subtree the CPU cannot is two evaluators computing two worlds. D690: `compile_field` takes a SET of roots — a `Script` names **686** on the estate and compiling from the solid alone leaves 685 aimed at whatever now occupies their old index, which is a building painted from the wrong shapes with no error anywhere. 18,250 nodes → 8,536, depth 146 → 134, 1.191x, 0 sign flips over 28,224 points; the transform fold measures 0.887x and stays OUT, D637 and D682's mistake for the third time. **It shipped OFF for one reason and that reason is gone — D705, and it is ON now.** `--part` refused on any clip with an `origin` because `apply_origin` wraps every named part in a fresh translate nothing else references, so 0 of 5,091 names survived. Handing the names in as ROOTS would have fixed it and gutted the rewrite, because `gather` only flattens a child with one parent and flatten-and-re-balance IS the 1.19x — proved on a field small enough to count by hand: as a name, 4 nodes and 1 flatten; as a root, 5 nodes and 0 flattens. So names go in as a SECOND list that takes no part in `survey` and is emitted after every root against the table the roots left behind: **5,091 of 5,091 kept, nought on the walk, and the building node-for-node identical to one compiled with no names at all.** And it found a **FIFTH** index that builds — `script.variation.by`, missing from the root set, which a control arm shows as 51 failed assertions and 3,603 distinct records against 15,722. The estate has no `by=`, which is why nothing caught it. The older note: the sampler ships OFF behind `--gpu-sample`, and the reason is a measurement (D677).** The clip's field, its parameters and the plan's widened rules go to the card (`src/gpu/field_gpu.*`) and `shaders/field_types.glsl` + `field_leaf.glsl` + `field_walk.glsl` + `sample_field.comp` evaluate it there. **The gate is `--gpu-sample-check N`**, which samples the nodes THE LADDER ITSELF PICKED a second time on the CPU through the same `SamplePlan` and compares cell for cell: on the estate -- seven buildings, 628 paint rules, an 18,250-node field 80 deep -- **48 nodes and 24,576 cells, 0 differing in matter, 0 in material, 0 in the clip mask**. That is D200k met on the real building rather than on a small clip, and unlike `clips/sampler.clip` it cannot pass vacuously. **It is 1.30x and that is why it is off**: matched 45-second arms, same camera and batch, the card delivered 87,680 nodes against the CPU's 66,432 -- for **35-60 ms of the card per dispatch**, once a frame, against a frame the renderer wants in 17, and `--refine-batch 2048` LOSES THE DEVICE to the driver's watchdog. A third more nodes for two thirds of every frame is not a trade `09-performance-budgets.md` allows. **`--gpu-visits` says where the rest is, counted rather than reasoned about: a cell WALKS 4,158 nodes of an 18,250-node field.** So the box cull is working and the walk is simply long -- and the CPU's whole advantage is that it evaluates about TEN TIMES FEWER POINTS, which is what `forge::sample`'s `descend` is. **AND THEN D678 REFUTED THE STEP THIS ROW NAMED, so read that before believing the sentence after this one.** The gate already samples the same nodes both ways, so it was asked directly rather than inferred from two throughput figures: over 24,576 cells of nodes the LADDER picked, the CPU's descent came down to a single voxel **24,576 times, 100% of them**, and made 1.14 shape evaluations a cell against 152 paint evaluations in total. A settle saves the cells it does not ask about and that is NONE of them -- the ladder only picks nodes that straddle a surface, `box_may_hold_matter` refuses the rest, and a node is eight cells a side. **The descent's saving is real and is spent before the ladder hands anything over.** What is left is the WALK, and both arms pay it: 4,158 node visits a cell over about two evaluations of an 18,250-node field is ~2,000 nodes an evaluation, and the CPU walks the same tree. The suspect is already counted -- D675: **923 of those nodes (25%) carry no box**, and an ancestor of an unbounded node cannot be bounded either -- so the next stage is to find out why, and it makes the CPU arm faster by the same factor. **The sampler is also ON now (D678): the trade -- a sharper world sooner against a slower picture while it fills -- was put to the user and taken.** `kSafeBoxes` caps a submission at 256 nodes whatever `--refine-batch` says, because past the watchdog the device is lost rather than slow. What follows was written before that measurement: ~~**c is two lettered steps and neither reaches the hundredfold alone: the DESCENT on the card** (one workgroup per node, 512 threads, a settle in shared memory at the node, its eight octants and its sixty-four sub-boxes; `prune_root`, `prune_slack` and `slack` are already in the plan and only need uploading; expected to recover most of the 10x and put the card at 6-8x, which is where it can be defaulted on), **and then COMPILING THE FIELD FOR THE CARD** -- fold transform chains into the leaves and build the hierarchy over wide unions that `Field::Accelerator` already knows how to make and that D675 counted at only **19 of 209**.~~ **Three findings that outlive their diffs.** A GLSL helper containing a call is a COPY of everything it calls: `sample_field.comp` had seventeen calls to `field_eval`, GLSL has no out-of-line call and the build compiles with `-O`, so that was seventeen private copies of a 96-frame stack -- 98 KB of scratch a lane, **1.05 MB of SPIR-V and 5.8 s of glslc against 286 KB and 0.7 s** once it became one call inside a phase machine. `atan(0,0)` is UNDEFINED in GLSL where `std::atan2(0,0)` is 0, and every op measuring an angle about an axis reaches it for a shape centred on a grid line -- a NaN in a distance field propagates through every min and max above it. And **`Field::eval` and `Field::mirror_eval` do not agree about everything** (the box cull, `blend`'s clamp); the shader follows `eval`, because `eval` is what `forge::sample` calls and therefore what the gate compares against. **The method note: the first arms said the card was 1.7x SLOWER and were not comparable** -- a 150-second run against a 75-second one is two different node populations, because deeper nodes are finer and dearer. Trap 8 wearing a wall clock; match the WINDOW as well as the camera. What follows is the stage as it was written before any of it was built. **It is R11's successor rather than a parallel option.** After R11 the CPU round trip — miss, report, sample, paste, upload — is the last thing between a ray and its geometry. `20-clip-forge.md` §4 already names the GPU as the answer and says why it transliterates. It also unblocks R2b's unfinished half, which has been stuck since D259 because eviction can only drop what it can afford to rebuild |
...
in the engine to switch on. So every rule below is a function of two bytes read as quantities: bins
from roughness and coverage, a lobe width from roughness, a tint from metalness. The plan's *no
roughness threshold anywhere, no `kSpecularSharp`, no `kSpecularBlurry`* is that sentence said from
the other end, and it applies to the instruments as well — a census that counts "faces past a
cutoff" is where a cutoff comes from.

**R4a's first half is done** (D582): a face knows what it is made of. See §8.0 and the note there
about coarse faces, which have no one material and say so.

- **R4a — the variable payload**, allocated only where it is read; matte stone allocates none.
  **Both halves are in**: the material (D582) and the payload, which is R4c's pool of blocks.
- **R4b — bins from measured pixel coverage and roughness** (D186). No roughness threshold.
  **D592 adds a second half to this that D186 did not know about**: a face given many bins also
  needs a ray drawn along its lobe, because the far ray is cosine-weighted and the bins a
  reflection is read out of are the grazing ones a cosine ray barely reaches.
- **R4c — outgoing bins**: reflection, read by the composite. **Done, with the image owed** —
  D591, D592.
...
off-screen face held nothing.

**The paragraph that used to be here said R9c was the whole of what was left in that measurement, and
it was wrong** (D569). It read: *with the class lit, the bucket R9c addresses is unchanged at 21.9% of
gathering rays landing on a surface with no face in the store at all, and it is the largest remaining
source of black in the bounce.* The first clause is a measurement and the second is an inference from
it, and nobody had tested the inference. Three run-time dials about **claim throughput** —
`--secondary-period`, `--secondary-share` and `--face-pressure-from` — take that bucket from 21.8% to
**8.7%** and the rays landing on a lit face from 29.9% to **41.5%**, without a line of new code. A
halo cannot move a number those dials move by two thirds: most of that bucket was surfaces a
gathering ray *does* name, whose claim was declined by a cap or evicted before the next ray arrived.
D570 and D571 took the first part of it (the cap), and D572 records what is left and why the obvious
dial was not turned.
...
>
> *(That is a correction, and the sentence it replaces was "no light path may cause streaming",
> stated as absolute with R9a as the single deliberate exception. R9i is a second one and it
> postdates the sentence: a shadow ray reports the cell that stopped it, D341–D343, and D430 lets it
> say it is using that cell. Both are deliberate and both are bounded by `node_seen` (D431).
> Measured over a settled run: the pool built **1 node** with the light rules on and 4 with
> `--no-light-keeps-geometry`. An absolute the code deliberately breaks in two places is worse than
> no rule at all — the next reader either "fixes" a working mechanism or stops believing the file.
> D589.)*
...
- **R5a — a-trous over the face lattice. Done — D573, D574, D576.** Not kept from the tracer after all,
  which is what R3d deleting it settled: the tracer's was a screen-space filter and this is over the
  face key, which is a different and much cheaper thing. A face's `open_sky` and its bounce — the two
  ...
  **Three terms**, chosen from a photograph of every term on its own taken before any of this was
  ...
  deliberately not filtered — a shadow edge is a real high-frequency feature — and neither is the
  near field, which that photograph says is the smoothest term there is (6.0 of 255 at the enclosed
  camera against the lamps' 23.0).

  Measured, two interleaved rounds, `--no-face-denoise` the control: face-to-face **roughness
  4.3539 → 2.9693** at the steps (−32%) and **3.0087 → 1.7292** enclosed (−43%), **speckle
  35.20 → 27.53** and **12.11 → 7.99**, with the mean pixel moving 0.02–0.19 of 255 — inside the
  run-to-run floor, which is what a filter that only takes variance out should do. The lamp tap is
  more than half of the enclosed figures on its own. Walking out of the room and back: roughness
...
- **R5b — temporal EMA per face and parent seeding at claim time. Half built, and OFF —
  ...
  **...and the other half of the same sentence is now built and is ON — D665.** Seeding decides
  where a new face's estimator STARTS; what was never decided is how far it is BELIEVED while it is
  still short, and `kFaceSettled` = 1 answers *outright, from one ray*. So a reader now blends a
  face's own sun ratio towards the coarse face standing over it in proportion to its own sample
  count, reaching full confidence at `kFaceEager` = 4 — which is exactly the population the audit
  calls unsettled (**135,071 of 589,870, 23%**, close camera) and is early enough that a settled
  shadow edge is untouched to the bit, so this does not become the cross-face filter R5a
  deliberately refuses the sun. It is `bounce_confidence`'s shape on the one term that never had it,
  **and it casts no rays** (D394): one word a face — `kFaceLightWords` 20 → 21, +4.2 MB — written
  where the counters are written and read by the composite and by a gathering ray out of a record
  both already load. `--no-sun-confidence` is the control arm and is spelled in the WRITER, because
  `resolve.comp` has no binding for the probe buffer; cleared, both readers return the raw ratio
  bit-exactly. `tests/test_sun_confidence.cpp` reads the constant out of the shader and holds it
  against the store's.

  **The pictures on this box do not resolve it and a COUNTER does, which is the useful half.** Three
  runs an arm of `clips/beam_test.clip` put the two arms inside each other — speckle 393.24 against
  395.73 with one arm's own runs spanning 3.85, trap 9 and D660's shape. But every differing pixel
  was a full flip between black and white, which a blend never produces, so the ramp was not firing
  on them: `kProbeSunRamped` / `kProbeSunNoStandIn` (the `sun's confidence ramp:` audit line) read
  **4,665 faces short of four samples, 3,006 of them — 64.4% — refused a stand-in** because the
  read path had inherited `kSunSeedMin` = 16 from the one-time claim seed. A prior written once and
  a blend re-decided every frame are different questions; `kSunStandInMin` = 4 is its own constant
  now, and refusal falls **64.4% → 54.4%** with faces served **1,659 → 2,128** for no rays. Trap 16
  is the rule that found it. The facility could not be run here — a card-free facility render peaks
  at 9.2 GB against a 13.3 GB cgroup shared with every other session, and three attempts were
  OOM-killed.
- **R5c — deterministic two-level blend** in the composite; delete the ordered dither.
- **R5d — analytic edge AA. The FIRST HALF is in — D663.** A primary ray that stops on a node
  coarser than a voxel, and whose per-direction coverage says it is not full, now marches on past
  that node; the composite draws the two surfaces in proportion. So a coarse node has stopped being
  a solid block, which it has been since this renderer existed.

  **The mechanism is R4d's second march and not a second one of its own**, which is most of why this
  is small: the far surface is packed into the same `in_behind` image, with the same face slot, the
  same folded colour, the same wire format, read by the same twenty lines of `resolve.comp`. One bit
  (18) says which of the two questions was asked, and it is needed because the two blends are not
  the same blend — a pane's transmission scales the near surface's DIFFUSE and leaves its reflection
  and its emission alone, where a share of a pixel scales all three, since over that share there is
  no surface to have a highlight.

  Three things were built to make it right rather than merely present.
  `NodeHit::coverage_exit_t` is where the ray leaves the cell the coverage was read off, taken free
  from the DDA's own `t_max`; it is **nought at the two other places `coverage` is set**, because
  both of those read the byte off a node coarser than the cell they draw and blending a pixel by a
  fraction measured over eight times its footprint is trap 6 at a different scale. `node_march`
  gained a **`start_t`**, so the second march skips forward along the ray instead of relaunching
  from the silhouette — relaunching restarts the detail clock, and a ray that thinks the eye is at
  the silhouette resolves the landscape behind it in bricks and asks the pool to **build** them.
  And `kEdgeCoverageFull` = 251 skips the march for a node 63/64 covered, where the blend is under
  the quantisation of the byte it would be packed into.

  **The cost is bounded by the geometry rather than by a dial**: a flat wall struck face on has
  every column of its projection covered, reports 255 and pays nothing, so what pays is silhouettes
  and open structure — which is the set that needed anti-aliasing.

  **Measured, card-free, one build and one flag** — `clips/edge_aa_test.clip` (written for this,
  because every clip small enough for a software rasteriser is four metres across and contains no
  coarse pixel at all), 320×200, `--settle`, shot at measured frame 40, both arms on scene
  `047acb7e41e2668b`, `--no-auto-exposure` because the light meter otherwise moves the whole frame
  by 2.6× against 2.7× and swamps it. See D663 for the figures and for the two things that had to be
  controlled for.

  **What is not built**: the plan's *up to three partial hits*. `out_behind` carries one surface a
  pixel, so a partly covered node behind a partly covered node is drawn opaque — which errs towards
  the picture this replaces rather than past it. And the coverage FOLD is a maximum over children
  (`NodePool::fold_children`), so above the brick a node reads as more solid than it is: a railing
  gets its blend and a silhouette across a node that is half solid wall gets none.
...
- **the light meter. Done — D577, D578**, and it was not in this list because when the list was
  written the tracer still had one. R3d deleted the tracer and R1e the frame-statistics buffer under
  it, so `resolve.comp`'s `kPreviewExposure` has been the constant 3.2 **with no writer at all**
  since — which is why `clips/many_lamps.clip` came out blown white (mean pixel **248.9 of 255**,
  99.9% of the frame over 200) and `clips/exposure_range.clip`, a room with one window, came out
  crushed black (**35.8**). Both are clips somebody wrote to test exactly this and could not use.
  They read **150.6** and **149.3** now, and the meter chose 0.187× and 33.178× to do it.

  The arithmetic is `pt_post.glsl`'s, which §9 kept for this: a log-average over whole workgroups,
  eased in stops at half a second, two slots so every invocation reads a frame nobody is writing.
  What is added is `kExposureBias`, +1.3 stops, a second constant rather than a bigger
  `kMiddleGrey` — the meter is a measurement and the compensation is a look, and the facility's
  three cameras move by 2–6% instead of 35% because of it. `--no-auto-exposure` restores the
  constant exactly, by driving the shader down its own "nothing measured" branch rather than by a
  second code path.
...
- **R11a — one node, sampled, timed. Nothing after this may be sized without it.** The primitive is
  ...
  **DONE, and the gate failed before it passed.** D613. The mapping is one place —
  `forge::node_sample_settings`, `node_voxels_per_metre` and `node_box`, with every node eight
  voxels a side at every level — and the instrument is `--sample-cost`, which returns non-zero on a
  disagreement rather than printing one. It found the sampler disagreeing with **itself**: 17 of 32
  nodes at level 8 and 1 of 32 at level 7 came out different sampled alone and sampled inside the
  building, because a box could be settled empty in bulk without allowing for the thin-feature
  rescue's own reach. Fixed at the source (`kHalfCellDiagonal`, one constant in one place), and the
  brute-force test now runs at 1, 2, 4, 8 and 16 voxels a metre as well as at 32.

  The numbers the rest of this stage is sized against, facility, eight workers:

  | level | node | voxels/m | with matter | one node | empty node | one at a time costs |
  |---|---|---|---|---|---|---|
...
- **R11b — the unit of refinement is a node, not a region.** ~~Delete `plan_refine_regions` and its
  eighteen boxes.~~ **DONE** (D615), with the facility gate outstanding — see below.
  Delete `plan_refine_regions` and its eighteen boxes. The queue is fed by adaptive subdivision from the clip's bounds, ordered by the
  ...
  **Done, and the gate is met on a small clip rather than on the facility.** The list is seeded at
  four-metre nodes and a node splits into eight when it is more than a quarter of its own distance
  across — the projected size the ladder already ranked by, so grain and order are one rule. On
  `clips/sampler.clip` with `--refine-all`, the node ladder and the eighteen-box ladder produce
  **byte-identical worlds** with despeckling off (`a1f8bc6c656343b7`, 1,430,104 solid voxels in
  both) and identical geometry with it on. What differs is despeckling's edge voxels: **115 cells
  of 49,152 on that clip, 46 of 152,064 on the facility**. The facility's own A/B is a ten-minute
  run a side and has not been done.

  Three pieces came with it: a node is skipped only when the WORLD has nothing to replace **and**
  the FIELD says a sample could find nothing (the world half alone lost 4,923 voxels); the cache
  stores sharpened boxes and reads them back by containment, because a camera-dependent grid has
  no stable index; and despeckle's whole-clip judgement is separated into `forge::StippleVerdict`
  and taken once. `--refine-all` exists because two arms that stop in different places cannot be
  compared.

  **Built, measured and reverted: a one-voxel skirt** so a node's edge voxels could see their true
  neighbours while despeckling. It lost **240 voxels of 1.43 million**, and `--sample-cost` says
  why: a box one voxel larger on every side is not the same question as an aligned one — **2 of 96
  nodes differ** on `sampler.clip`, 0 of 297 on the facility. D613's class one step along, and
  R11c will want it fixed.
...
- **R11c — the resolution is the level's, not one of two constants.** `256 / 2^level`, so a node
  ...
  **DONE, and that gate is the part not measured** (D616). The world is proved instead: forced to
  full detail on `clips/sampler.clip`, the level ladder produces a **byte-identical** world to the
  eighteen-box one through 9,819 samples at six different resolutions. What the fly-in would add is
  whether the two remaining doublings — 8 → 16 → 32 voxels a metre as you approach — read as steps
  to an eye. Three things had to come with it: a node too coarse to improve on the world **splits**
  rather than finishing (the first version refined four nodes and called the world settled); a node
  never overwrites detail finer than its own; and the split loop **follows the winner down** rather
  than re-picking from the whole list, which cuts everything finer and samples nothing.
...
- **R11d — nothing is sampled up front.** The first box is on demand like every other. `--clip-coarse`
  ...
  **DONE AND ON BY DEFAULT — D673. The gate above is met: 292, 297 and 309 ms to playable over three
  runs on the ESTATE with no cache, against the 500 ms asked for and the 3,615 ms of D611** — and the
  estate is seven buildings where D611's facility was one. The control arm, `--coarse-paste
  --stipple-at-coarse`, was abandoned at 80% of 261 M voxels with its own estimate reading forty
  minutes.

  **It was built weeks before it shipped, and what kept it off was a sentence.** The condition is
  `!no_coarse_paste || stipple_at_coarse` — two flags — and every heading, summary and ledger row
  naming the feature named `--no-coarse-paste` alone. That flag skips the paste and leaves the
  sample, which is the entire bar. See D673; the same entry closes three faults the flip exposed (the
  world clean ran at only one of the two fixed points, a world was never kept when the player left
  one, and the cache key did not record which arm built it).

  **The history below is R11d's first form — D630 to D635, and read them before touching it.**
  The up-front sample is **taken and not pasted**: the ladder builds the world from nothing, seeded
  at eight metres. Five things were asked of it and it answers all five — the world grows when the
  camera moves (19,751,324 → **29,622,982** voxels walking the same scene, D632); a camera-dependent
  world is reproducible (same camera twice, **`70b51a3f94dc44ba`** to the digit, D633);
  `baseline.ps1` needed no change (D633); a partial world resumes elsewhere and accumulates (D634);
  and a chisel cuts properly with nothing under it (D635).
...
- **R11g — `--settle` and the harness mean what they say.** Traps 8 and 19, and this is the sub-step
  ...
  **The gate is SATISFIED and it needed no work — D633.** Measured against `--no-coarse-paste`, the
  first genuinely camera-dependent world this engine has built: the enclosed camera gives
  **`70b51a3f94dc44ba` and 19,751,324 voxels twice over**, and the close camera gives
  `42af22ca9c2820a5` and 3,631,394. The same camera reproduces to the digit and a different one does
  not, which is exactly the property the gate wants. `baseline.ps1` matches its row by (mode, view,
  size) and **never compares across cameras**, so the harness had no opinion to correct. `--settle`
  generalises as this entry hoped — *"refinement has nothing left it can do from here"* is a fixed
  point of the camera — and **D627's stand-down is what makes it a fixed point rather than a
  timeout**. What remains true: a baseline taken with the coarse paste and one taken without are not
  comparable row for row, and the gate will say so, correctly.
...
- **R11h — an edit needs voxels that may never have been sampled.** Today the world holds real
  ...
  **The NEAR case passes — D635.** A 20 m cube carved through the middle of the facility under
  `--no-coarse-paste`: the cube is cut, its faces are clean and sharp rather than stepped, and the
  portico revealed behind it is built at full detail. Voxels go **19,751,324 → 25,232,463** and the
  rise is the carve working — the hole opens the building to the sky, so what was occluded becomes
  visible and the ladder builds it. **`0 leaves rebuilt` on that arm is about the POOL and not about
  the edit** (it counts leaves the pool already held and had to re-derive, and this arm held far
  fewer); it nearly condemned a working carve. **The FAR case is the half that is left** and is what
  this sub-step is really about: sixty metres into a surface never approached, where the proximity
  radius has to hold *sampling* rather than *residency*. That arm is not run.
...
**DONE, and what the stage looks like after it — read D677 before any of the lettered steps below,
because two of them are finished and one has split in half.**

| | | |
|---|---|---|
| **R12a** | the field as an array, uploaded | **DONE.** `src/gpu/field_gpu.*`; the estate is 18,250 nodes and 1,425 KB |
| **R12b** | `shaders/field.glsl`, the evaluator | **DONE, and the mirror gate passes on the ESTATE.** Three files, and `--gpu-sample-check` |
| **R12c** | the marcher derives rather than stops | **BUILT, gated, and OFF — D699.** A descent that reaches an unbuilt node evaluates the field there and draws it, at the level the pixel resolves, instead of R2d's stand-in; a derived cell that comes back AIR carries the march on, which is what makes it a building against sky rather than a blob. The evaluator is R12b's, included unedited and moved to descriptor set 1 by four lines of preprocessor. **Three budgets, and two of them each cost a device**: a cap on CELLS bounds nothing (D688's spread is two hundredfold), a visit budget read after the fact bounds nothing (every lane reads nought), and ONE CELL had to be bounded on its own at ~1,272 field evaluations. Enclosed 7.57 ms against 6.60, outdoor 10.15 against 6.33, worst frames 156–1383 ms; uncapped it is 344 ms mean and a 38-second frame. **The settled picture is identical, same content hash both arms.** D687's thirty-times verdict stands. **The missing half: the first cold frame cannot be derived from inside the marcher** — no chunks means no roots means nothing WANTED, and seeding the pool from the clip's extent is a `node_pool` change |
| **R12d** | the CPU sampler writes what is EDITED | not started |

**And the two steps that were not in this list and have to be, because 1.30x is what the stage buys
without them:** the DESCENT on the card, and COMPILING THE FIELD for the card. Both are in the §8.0
row above with their measurements. A cell currently walks 4,158 nodes of an 18,250-node field, and
the CPU keeps up by evaluating ten times fewer points rather than by being faster at any one.
...
| ~~**A camera-dependent world breaks every measurement in the repository**~~ **— measured, and it does not. D633** | two runs of one build at one camera disagree on the content hash, so no A/B in this file gates any more — and it reads as flakiness rather than as a design consequence | **The fear was wrong and the row is kept for the reason.** Against the first genuinely camera-dependent world (`--no-coarse-paste`), one camera reproduces **`70b51a3f94dc44ba` to the digit twice** and a second camera differs — the dependence is *between* views and `baseline.ps1` matches its row by (mode, view, size), so it never compares across cameras and **needed no change**. `--settle` generalised as R11g hoped; D627's stand-down is what turned it from a timeout into a fixed point. Still true: a CSV taken with the coarse paste and one without are not comparable row for row |
| **A chisel lands on a surface that has only ever been seen from sixty metres** — **near case measured and clean (D635); the far case is what is left** | the player carves a doorway and gets a four-voxel-wide blocky hole, permanently, because R11f makes carved matter authoritative | **R11h.** The proximity radius already exists and already holds twenty metres at full detail (R2c, D199); it holds *residency* today and must hold *sampling* after R11d. A 20 m cube carved under `--no-coarse-paste` cuts clean sharp faces and the portico it reveals is built at full detail; **sixty metres into a surface never approached is not run** |
