...
| R2 | b. the sub-pixel rule | **half, and the half that is left is large** — D259–D261. A node finer than the pixel is never *requested*; making it never *stored* needs per-node residency, because eviction currently works at the 512 m root and the whole scene is one root. Gate measured at 3.2× over |
