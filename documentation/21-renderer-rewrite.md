...
| R4 | b, d | not started. **R4b is now sized by D592 rather than by D186 alone**: the bin count has to follow pixel coverage AND the faces that get many bins need a ray drawn along the lobe, which the far ray cannot be — `open_sky` and the bounce are its other two answers and both need the cosine. That is a second ray for one face in nineteen |
| R6a–R8 | | not started |
