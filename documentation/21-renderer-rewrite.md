...
| R1 | e. delete the old addressing | **four slices in, not finished.** (1) One marcher: `--chunk-marcher`, `--node-pool`, F6 and `use_node_pool_` gone; `node_visibility.comp` renamed `visibility.comp`. (2) **`shaders/world.glsl` deleted** — 793 lines, no includer left, and `grid_dims` proves the point: still declared in `params.glsl`, read by no shader in the tree. (3) **The summary octree and eight thumbnail tiers deleted** — `summary_tree.*`, `thumb_cache.*`, `thumbnail.*` and their three test files, 1,180 lines; suite 548 → 530 because the tests are DELETED rather than disabled. (4) The ray clip box now comes from `world_min_/world_max_` rather than `residency_.resident_bounds()`, which was **the last thing the marcher still read out of the chunk system** and the reason `residency.*` could not go. **Left: `residency.*` (1,329 lines), most of `gpu/world_buffers.*`, the chunk marcher's orphaned descriptor set, and `rebuild_coarse_grids` at 4.10 ms an edit.** Content hash `1f4710eee4ee2585` unmoved at every step, validation clean, both pool audits clean. **The stated gate has NOT been run**: `toolsaseline.ps1 -Compare documentationaselines2-node-pool.csv` |
