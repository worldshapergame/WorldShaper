...
| R5 | a. a-trous over the face lattice | **done** — D573, D574. Nothing in this renderer filtered across faces, which is the class the reported fine grid on flat surfaces belongs to (D539 eliminated eight causes and found no ninth because there was not one). A face's `open_sky` and bounce are blended with its **coplanar** neighbours' in a 3×3 tent weighted by how well measured each is — and there is no edge-stopping term at all, because a neighbour at the same level and direction one step along a tangent axis is coplanar and contiguous *by construction*, so what a screen-space denoiser spends most of its arithmetic on does not arise. Two interleaved rounds against `--no-face-denoise`: **roughness 4.3515 → 3.1410** at the steps and 3.0165 → 2.4458 enclosed, **speckle 35.27 → 28.69** and 12.12 → 9.98, mean pixel moving 0.02–0.10 of 255. Walking out and back: 3.2157 → 2.6859 and 13.210 → 10.847. Flying, the arms interleave. It reads words 0–11 and writes 12–15, so a filter that filters its own output is unrepresentable rather than avoided, and only the composite reads them — a gathering ray takes the raw bounce, or the blur compounds through the light transport instead of through the buffer. Costs 3.52 → 3.87 ms settled and **16.5 MB**. **The lamps are still unfiltered and are the noisiest term indoors** |
| R4, R6–R8 | | not started |
...
- **R5a — a-trous over the face lattice. Done — D573, D574.** Not kept from the tracer after all,
  ...
  **There is no edge-stopping term and that is the whole saving.** A neighbour at the same level and
  ...
  4.3515 → 3.1410** at the steps (−28%) and 3.0165 → 2.4458 enclosed (−19%), **speckle 35.27 → 28.69**
  and 12.12 → 9.98, with the mean pixel moving 0.02–0.10 of 255 — inside the run-to-run floor, which
  is what a filter that only takes variance out should do. Walking out of the room and back:
  roughness 3.2157 → 2.6859, speckle 13.210 → 10.847. Flying, the arms sit inside each other's
  spread. It costs the faces pass 3.52 → 3.87 ms settled at the steps and **four words a face**
  (50,688 → 67,584 KB).

  The sun is deliberately not filtered — a shadow edge is a real high-frequency feature — and neither
  is the near field, which was photographed on its own first and is the smoothest term there is.
  What is still owed here is the LAMPS, which that photograph says are the noisiest term indoors
  (speckle 23.0 of 255 against the near field's 6.0).
