...
5. **After every change: build, test, COMMIT, and merge it to `main` on GitHub.** Not at the end of a
   ...
   **A release is the same loop with a tag on the end.** `.github/workflows/release.yml` had never
   ...
   now and the workflow builds the facility out of its own zip before publishing (D641) — **and it
   has now gone green: v0.7.1 was built and published by it in five minutes.** `tools/package.ps1`
   remains the path for a zip you do not want published,
...
| R7 the primary ray | L | **a and b DONE — D698.** The beam pre-pass, one coarse ray per 8x8 tile CORNER, and a temporal start distance BOUNDED by it so a wrong reprojection can only cost time. 4K, arm to arm: **1.18 ms against 16.81 outdoors, 1.96 against 15.07 close**; 0.76 against 0.56 at distant, where the ray was already trivial. **c is not done** — step count in the descent |
| R8 infinite detail | XL | **b is DONE — D696**, the hashed child source that is always available: a hash over the TREE rather than over a sub-voxel coordinate, so there is nothing to overflow and "no cap" is a property of the construction. Mirrored against the whole facility at **0 disagreements**, deterministic across runs. **a (signed levels) and e (`--infinite-detail`) are not done**, and R8b is inert until one call site in `node.glsl` and one in `resolve.comp` move — the patch is written and was handed back. **Re-sized to L**: R8c and R8d moved into R11 (D612) |
...
| R12 the field on the card | L | **a, b and c are DONE and BOTH named speed steps with them — D690, D691, D699.** The card was missing two of `Field::eval`'s three union culls, so a cell now walks **2,876 nodes against 8,273** and on a quiet machine the card is AHEAD of the CPU arm on the camera D681 measured it 2.7x behind on. The compiler takes a SET of roots (686 on the estate) and is 1.19x. **R12c derives rather than stops and ships OFF**: D687's thirty-times verdict stands, and today took about 3.3x of that thirty. **d is not done**, and neither is R12c's own missing half — the first cold frame, which is a `node_pool` change |
...
| **Every cut in the facility audited, and the instrument corrected twice.** `clipcheck --cuts` names what each subtraction removed and WHICH PART it took it from; a cut that removes nothing is reported as loudly as one that removes too much | **in** (D669). Two faults in the tool, both found by agents who blamed their own file first: `apply_origin` never moved the names a file bound, so **every `--part` measurement ever taken sampled an unmoved shape in a box shifted 3.50 m**; and `never fired` counted evaluations and was never switched on, so it had never printed — it now reports **30 of 365 rules matching no voxel**, `windows_glass` among them |
| **The openings were cut 1.80 m through a 0.90 m wall, all 56 of them** — 14.153 m³ of other fragments' rooms deleted, now 2.670. The stair lost 0.32 m³ out of its half landing in both wings; the west door bored 0.79 m³ out of the chapel | **in** (D669). The mirror failure too: `offset by=` is positive to shrink and BRIEF.md said otherwise, so every joint in the great stair did not exist — 2,423,674 voxels both sides of the difference, no error, no symptom |
